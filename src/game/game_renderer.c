 ...
 * game_renderer.c
 *
 * Handles all Draw* / Gui* calls for the active game screen.
 * Called exclusively from gameplay_core.c → GameDraw().
 *
 * No game-logic decisions live here — only visual presentation.
 */
...
/* Forward declaration for the drag overlay (lives in drag_drop.c) */
...
void GameRenderFrame(AppState *state)
{
    ...
    {
        DrawLineV((Vector2){boardX + (i * cellSize), boardY},
                  (Vector2){boardX + (i * cellSize), boardY + boardVisualSize},
                  (Color){54, 68, 82, 100});
        DrawLineV((Vector2){boardX, boardY + (i * cellSize)},
                  (Vector2){boardX + boardVisualSize, boardY + (i * cellSize)},
                  (Color){54, 68, 82, 100});
    }
    ...
    for (int r = 0; r < match->board.sideSize; r++)
    {
        ...
        {
            Tile placedTile = match->board.placedTiles[r][c];
            if (placedTile.letter != '\0')
            {
                ...
                DrawText(letterStr,
                         cellBounds.x + (cellSize - textW) / 2.0f,
                         cellBounds.y + (cellSize - tileFontSize) / 2.0f,
                         tileFontSize,
                         (Color){38, 28, 16, 255});
                ...
                DrawText(scoreStr,
                         cellBounds.x + cellSize - scoreW - (cellSize * 0.08f),
                         cellBounds.y + cellSize - scoreFontSize - (cellSize * 0.06f),
                         scoreFontSize,
                         (Color){80, 65, 50, 255});
            }
        }
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        if (i % 2 == 0)
        {
            DrawRectangleRec((Rectangle){detailsRect.x + 6.0f, currentRowY, halfRightWidth - 12.0f, rowHeight},
                             (Color){30, 38, 46, 180});
        }
    ...
    DrawLineV((Vector2){scoreRect.x + 12.0f, horizontalLineY},
              (Vector2){scoreRect.x + scoreRect.width - 12.0f, horizontalLineY},
              separatorColor);
    DrawLineV((Vector2){scoreRect.x + 15.0f + scoreColW, scoreRect.y + 20.0f},
              (Vector2){scoreRect.x + 15.0f + scoreColW, scoreRect.y + scoreRect.height - 12.0f},
              separatorColor);
    ...
    for (int p = 0; p < 2; p++)
    {
        ...
        for (int t = 0; t < match->players[p].rack_count; t++)
        {
            ...
            DrawText(letterStr,
                     ...
                     tileBounds.y + (tileSize - rackTileFontSize) / 2.0f,
                     rackTileFontSize,
                     (Color){38, 28, 16, 255});
            ...
            DrawText(scoreStr,
                     tileBounds.x + tileSize - MeasureText(scoreStr, scoreFontSize) - (tileSize * 0.12f),
    ...
    for (int p = 0; p < 2; p++)
    {
        Rectangle subHistoryRect = {historyOuterRect.x + 12.0f + (p * (historySplitWidth + layoutGap)),
                                     historyInnerY,
                                     historySplitWidth,
                                     historyPanelHeight - 40.0f};
        ...
        DrawText(TextFormat("Player %d History", p + 1),
                 subHistoryRect.x + 12, subHistoryRect.y + 7,
                 baseFontSize - 2,
                 (Color){196, 181, 137, 255});
        ...
        for (int w = 0; w < 0 && w < 5; w++)
        {
            ...
            const char *ptsStr = "+0";
            DrawText(ptsStr,
                     subHistoryRect.x + subHistoryRect.width - MeasureText(ptsStr, baseFontSize - 2) - 15.0f,
                     textY,
                     baseFontSize - 2,
                     (Color){111, 200, 140, 255});
        }
    }
